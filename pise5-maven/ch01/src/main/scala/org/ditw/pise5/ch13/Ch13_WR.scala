...
//  val addExpr = BinOp("+", Num(1), Var("y"))
  ...
  val ani: Animal = cat
  val cats: List[Cat] = List(Cat("kitten"))
  val anis: List[Animal] = cats
  // covariant
  // invariant
  val optCat: Opt[Cat] = Som(cat)
  val optAni: Opt[Animal] = optCat
  // java array covariant, 1.5 generic invariant
...
//  println(n2)
  ...
  // Product type: https://en.wikipedia.org/wiki/Algebraic_data_type
